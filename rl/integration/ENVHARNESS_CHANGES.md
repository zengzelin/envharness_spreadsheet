...
| `agent_system/multi_turn_rollout/rollout_loop.py` | Exports SpreadsheetBench parser, output-length, and structured Python error diagnostics into rollout batches. It also emits START/HEARTBEAT/END/ERROR boundaries around environment reset, model generation, and environment step so a stalled distributed rollout has a visible phase and turn. |
